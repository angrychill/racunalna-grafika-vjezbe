...
class CrtanjeWebGL {
        ...
        kombiniraj2DVrhoveIBoje(vrhovi:number[], boje:number[]) {
            var nova : number[] = [];
        ...
        kombiniraj3DVrhoveIBoje(vrhovi:number[], boje:number[]){
            var nova : number[] = [];
        ...
        kombinirajRed3DVrhovaIBoju(vrhovi:number[], boja:number[]){
            let red : number[] = [];
        ...
        vratiVrhoveKocke(a:number, centered:boolean) {
            var novi:number[] = [];
        ...
        vratiVrhoveKockeTrokut(a: number, centered: boolean) {
            let novi: number[] = [];
        ...
        vratiVrhoveGrid(min: number, max: number, step: number, boja: number[]) {
            let novi: number[] = [];
        ...
        vratiVrhoveValjka(r: number, h: number, n: number) {
          var vrhovi = []; 
        ...
        vratiMeshValjak(r: number, h: number, n: number): Mesh3D {
            let vrhovi: number[] = [];
            let offset = 0;
        ...
        vratiMeshStozac(r: number, h: number, n: number): Mesh3D {
            let vrhovi: number[] = [];
            let offset = 0;
            let drawModes: {mode: number, count: number, offset: number}[] = [];
        ...
        vratiMeshKugla(r: number, nLat: number, nLon: number): Mesh3D {
            let vrhovi: number[] = [];
            let offset = 0;
            let drawModes: {mode: number, count: number, offset: number}[] = [];
    ...
    for (let i = 0; i <= n; i++) {
        ...
        // vanjski rub
        vrhovi.push(
            r * Math.cos(phi), r * Math.sin(phi), -h / 2,
            0, 0, -1
        );

        // unutarnji rub
        vrhovi.push(
            r_inner * Math.cos(phi), r_inner * Math.sin(phi), -h / 2,
            0, 0, -1
        );
    }
    drawModes.push({
...
type Mesh3D = {
    ...
    drawModes: {mode: number, count: number, offset: number}[]; // kako crtati (TRIANGLE_FAN, TRIANGLE_STRIP)
};