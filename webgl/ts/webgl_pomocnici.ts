class WebGLPomocnici {
  ...
  static napuniSpremnike(gl: WebGL2RenderingContext, data: Float32Array): WebGLBuffer {
  ...
  static postaviAtribut(gl: WebGL2RenderingContext, program: WebGLProgram, attrName: string, size: number, stride: number, offset: number) {
    const loc = gl.getAttribLocation(program, attrName);
    gl.enableVertexAttribArray(loc);
    gl.vertexAttribPointer(loc, size, gl.FLOAT, false, stride, offset);
  }

  static iscrtaj(gl: WebGL2RenderingContext, mode: number, count: number) {
